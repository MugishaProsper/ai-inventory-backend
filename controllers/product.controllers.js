        ...
        if (stockStatus) {
            switch (stockStatus) {
                case 'out_of_stock':
                    ...
                    break;
                case 'low_stock':
                    query.$expr = { $and: [{ $gt: ['$quantity', 0] }, { $lte: ['$quantity', '$minStock'] }] };
                    break;
                case 'in_stock':
                    query.$expr = { $and: [{ $gt: ['$quantity', '$minStock'] }, { $lt: ['$quantity', '$maxStock'] }] };
                    break;
                case 'overstock':
        ...
        const summary = await Product.aggregate([
            { $match: { user: new mongoose.Types.ObjectId(userId) } },
            {
                $group: {
                    ...
                    lowStock: {
                        $sum: { $cond: [{ $and: [{ $gt: ['$quantity', 0] }, { $lte: ['$quantity', '$minStock'] }] }, 1, 0] }
                    }
                }
            }
        ]);
    ...
    try {
        ...
        if (!name || !price || !cost || !category || !supplier) {
            return res.status(400).json({
                success: false,
                message: "Missing required fields: name, price, cost, category, supplier"
            });
        }
...
export const updateProduct = async (req, res) => {
    try {
        ...
        if (updates.quantity !== undefined && updates.quantity !== existingProduct.quantity) {
...
export const deleteProduct = async (req, res) => {
    try {
        ...
        const inventory = await Inventory.findOne({ user: userId });
        if (inventory) {
            const productIndex = inventory.products.findIndex(p => 
...
export const updateProductStock = async (req, res) => {
    try {
        const { id: userId } = req.user;
        const { productId } = req.params;
        const { quantity, operation = 'set', reason = '', reference = '' } = req.body;
        ...
        const stockMovement = new StockMovement({
            product: productId,
            user: userId,
            type: operation === 'add' ? 'in' : operation === 'subtract' ? 'out' : 'adjustment',
            quantity: quantity,
        ...
        if (inventory) {
            ...
            if (inventoryProduct) {
                ...
                inventoryProduct.availableQuantity = product.quantity - inventoryProduct.reservedQuantity;
                await inventory.save();
            }
        }
...
export const getProductsNeedingReorder = async (req, res) => {
    try {
        ...
        const reorderSuggestions = products.map(product => ({
            ...
            supplier: product.supplier,
            suggestedQuantity: Math.max(product.maxStock - product.quantity, product.minStock * 2),
            urgency: product.quantity === 0 ? 'critical' : product.quantity <= product.minStock * 0.5 ? 'high' : 'medium',
