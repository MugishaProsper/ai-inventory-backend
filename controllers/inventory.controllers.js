  ...
  try {
    ...
    if (populate) {
      await inventory.populate(
        "products.product",
        "name sku price cost quantity minStock maxStock images status"
      );
    }
...
export const getInventorySummary = async (req, res) => {
  try {
    ...
    const inventory = await Inventory.findOne({ user: userId }).populate(
      "products.product",
      "name sku price cost quantity minStock maxStock category supplier"
    );
    ...
    for (const item of inventory.products) {
      if (item.product) {
        ...
        supplierBreakdown[supplierName].value +=
          item.product.price * item.quantity;
      }
    }
    ...
    const summary = {
      basic: {
        ...
        estimatedProfit:
          Math.round(
            (inventory.statistics.totalValue - inventory.statistics.totalCost) *
              100
          ) / 100,
      },
      stockStatus: {
        inStock:
          inventory.statistics.totalProducts -
          inventory.statistics.lowStockItems -
          inventory.statistics.outOfStockItems -
          inventory.statistics.overstockItems,
      ...
      alerts: {
        ...
        critical: inventory.alerts.filter((a) => a.severity === "critical")
          .length,
      },
    };
  ...
  try {
    ...
    if (!productId || !quantity || !fromLocation || !toLocation) {
      return res.status(400).json({
        success: false,
        message:
          "Product ID, quantity, from location, and to location are required",
      });
    }
