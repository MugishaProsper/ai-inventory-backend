...
export const getAllSuppliers = async (req, res) => {
    try {
        ...
        if (sortBy === 'performance') {
            ...
            sortOptions['performance.onTimeDeliveries'] = sortOrder === 'desc' ? -1 : 1;
        } else {
...
export const updateSupplier = async (req, res) => {
    try {
        ...
        if (updates.name || updates.contact?.email) {
            ...
            const orConditions = [];
            if (updates.name) {
                orConditions.push({ name: { $regex: `^${updates.name}$`, $options: 'i' } });
            }
                ...
                $addFields: {
                    ...
                    overallScore: {
                        $add: [
                            ...
                            { $multiply: [{ $divide: [{ $ifNull: ['$performance.qualityScore', 0] }, 100] }, 40] }
                        ]
                    }
                }
            },
