...
export const getDashboardAnalytics = async (req, res) => {
    try {
        ...
        const monthlyRevenueChange = ((monthlyRevenue - previousRevenue) / previousRevenue * 100);
        ...
        const categoryDistribution = await Product.aggregate([
            ...
            { $unwind: { path: '$categoryInfo', preserveNullAndEmptyArrays: true } },
            {
                $group: {
                    ...
                    color: { $first: { $ifNull: ['$categoryInfo.color', 'bg-gray-500'] } },
                    count: { $sum: 1 },
...
export const getInventoryAnalytics = async (req, res) => {
    try {
                    ...
                    stockStatus: {
                        $switch: {
                            branches: [
                                ...
                                { case: { $lte: ['$quantity', '$minStock'] }, then: 'low_stock' },
                                { case: { $gte: ['$quantity', '$maxStock'] }, then: 'overstock' }
                            ],
                            default: 'in_stock'
                        }
                    }
        ...
        const analyticsData = {
            ...
            abcAnalysis: abcAnalysis[0] || { abcCategories: { A: 0, B: 0, C: 0 }, totalValue: 0 },
            period
        };
...
export const getSalesAnalytics = async (req, res) => {
    try {
        ...
        const totalRevenue = products.reduce((sum, p) => sum + (p.statistics?.totalRevenue || 0), 0);
        const totalSold = products.reduce((sum, p) => sum + (p.statistics?.totalSold || 0), 0);
        ...
        const topProductsByRevenue = products
            .sort((a, b) => (b.statistics?.totalRevenue || 0) - (a.statistics?.totalRevenue || 0))
            .slice(0, 10)
            .map(p => ({
                id: p._id,
                name: p.name,
                ...
                profit: ((p.statistics?.totalRevenue || 0) - ((p.statistics?.totalSold || 0) * p.cost))
            }));
        ...
        const salesAnalytics = {
            summary: {
                ...
                averageOrderValue: totalSold > 0 ? Math.round((totalRevenue / totalSold) * 100) / 100 : 0,
            ...
            categoryPerformance: categoryPerformance.map(cat => ({
                ...
                averageRevenuePerProduct: cat.products > 0 ? Math.round((cat.revenue / cat.products) * 100) / 100 : 0
            })),
            period
        };
                    ...
                    deliveryPerformance: {
                        $cond: {
                            ...
                            then: 0,
                                ...
                                $multiply: [
                                    { $divide: ['$performance.onTimeDeliveries', '$performance.totalOrders'] },
                                    100
                                ]
                            }
                        }
